...
<body id="{$page.page_name}" class="{$page.body_classes|classnames} body-desktop-header-style-w-{$iqitTheme.h_layout} {if $customer.is_logged}customer_logged{/if}">
...
{if $page.page_name == 'index' || $page.page_name == 'authentication' && $logged !== 1}
    <script src="/js/inputStore.jquery.min.js"></script>
    <script>
          $('#customer-form input[name="firstname"]').inputStore();
          $('#customer-form input[name="lastname"]').inputStore();
          $('#customer-form input[name="email"]').inputStore();
    </script>
{/if}
...
<script>
var $container = $('.simpleblog__listing .grid');
