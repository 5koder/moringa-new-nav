...
class QuantityDiscountRule extends ObjectModel
{
    ...
    public function createAndRemoveRules($code = null, $context = null, $cache = true)
    {
        ...
        */
        ...
        if (!$this->array_search_partial(array_column($backtrace, 'file'), 'hsmultiaccessoriespro')) {
            if (in_array('removeQuantityDiscountCartRule', array_column($backtrace, 'function'))
                ...
                || in_array('_processCarrier', array_column($backtrace, 'function'))
                ...
                case 21:
                    ...
                    foreach ($cartProductsFiltered as $product) {
                        if ((int)$action->apply_discount_to_special || !Product::getPriceStatic($product['id_product'], (int)$action->reduction_tax, (isset($product['id_product_attribute']) ? (int)$product['id_product_attribute'] : null), 6, null, true, true, $product['cart_quantity']) > 0) {
                            ...
                            $totalAmount += $productPrice*$timesToApplyPromoInThisProduct;
                        }
                    }
    ...
    public function createCartRule($tempCartRule, $quantityDiscountRule)
    {
        ...
        if (isset($tempCartRule['gift_product'])) {
            ...
            //Flush cache, needed for validateOrder
            $this->context->cart->getDeliveryOptionList(null, true);
        }
                                ...
                                if (!(int)$condition->apply_discount_to_special) {
                                    ...
                                    foreach ($cartProducts as $cartProduct) {
                                        if (!($cartProduct['on_sale']
                                            || Product::getPriceStatic($cartProduct['id_product'], (int)$condition->cart_amount_tax, (isset($cartProduct['id_product_attribute']) ? (int)$cartProduct['id_product_attribute'] : null), 6, null, true, true, $cartProduct['cart_quantity']) > 0)) {
            ...
            foreach ($conditions as $condition) {
                ...
                switch ((int)$condition->id_type) {
                    ...
                    case 10:
                        ...
                        //If you define more than 1 condition from this type, if it has been already evaluated as true, the next time shouldn't be reevaluated
                        if ($previousChance) {
                            $groupValidationPassed = true;
                            break;
                        }
        ...
        if (!Cache::isStored($cache_key) || !$use_cache) {
            ...
            foreach ($cartProducts as $key => &$cartProduct) {
                ...
                if ($object->filter_by_stock) {
                    ...
                    $operator = (get_class($object) == 'QuantityDiscountRuleAction' ? $object->stock : $object->product_stock_operator);
                ...
                if (isset($cartProduct['productmega'])) {
                    foreach ($cartProduct['productmega'] as $productmega) {
                        ...
                        if (!isset($cartProduct['price_without_reduction_without_tax'])) {
    ...
    public function getCarrier($context)
    {
        if (Tools::getValue('delivery_option')) {
            ...
            //Flush cache
            $context->cart->getDeliveryOptionList(null, true);
            $context->cart->getDeliveryOption(null, false, false);
        }
