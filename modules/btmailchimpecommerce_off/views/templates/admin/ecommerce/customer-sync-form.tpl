...
<div id="bt_div_customer_sync_form">
	<div id="{$sModuleName|escape:'htmlall':'UTF-8'}" class="bootstrap">
		<script type="text/javascript">
			{literal}
			var aCustomerSync = {
				'sURI' : '{/literal}{$sURI nofilter}{literal}',
				'sParams' : '{/literal}{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.customerSync.action|escape:'htmlall':'UTF-8'}&sType={$aQueryParams.customerSync.type|escape:'htmlall':'UTF-8'}{literal}',
				...
				'oCallback' : [{
					...
					'params' : '{/literal}{$sCtrlParamName|escape:'htmlall':'UTF-8'}{literal}={/literal}{$sController|escape:'htmlall':'UTF-8'}{literal}&sAction=display&sType=syncForm&type=customer',
					'toShow' : 'bt_div_customer_sync_form',
					'toHide' : 'bt_div_customer_sync_form',
					'bFancybox' : false,
		...
		<form class="form-horizontal col-xs-12 col-sm-12 col-md-12 col-lg-12 fancyform" action="{$sURI|escape:'htmlall':'UTF-8'}" method="post" id="bt_customer_sync_form" name="bt_customer_sync_form">
			<input type="hidden" name="sAction" value="{$aQueryParams.customerSync.action|escape:'htmlall':'UTF-8'}" />
			<input type="hidden" name="sType" value="{$aQueryParams.customerSync.type|escape:'htmlall':'UTF-8'}" />

			{* use case - first sync *}
			<h2>{l s='Customers synchronization' mod='btmailchimpecommerce'}</h2>
				...
				<div class="form-group">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="alert alert-warning">
							{l s='Your customers have already been synchronized to MailChimp on the:' mod='btmailchimpecommerce'}&nbsp;<b>{$sDateCreated|escape:'htmlall':'UTF-8'}</b>  ({l s='Last update:' mod='btmailchimpecommerce'} {$sDateUpdated|escape:'htmlall':'UTF-8'})
							<div class="clr_10"></div>
							{l s='However, you can always manually resynchronize your customers by clicking on the green button below.' mod='btmailchimpecommerce'}
						</div>
					</div>
				</div>
			...
			{if !empty($iTotal)}
				...
				<h3>{l s='Preferences for customers synchronization' mod='btmailchimpecommerce'}</h3>
				<div class="clr_hr"></div>
				<div class="clr_10"></div>
				...
				<label class="control-label col-xs-12 col-sm-12 col-md-4 col-lg-4">
					<span class="label-tooltip" data-toggle="tooltip" title data-original-title="{l s='The count here indicates the number of customers to be exported'  mod='btmailchimpecommerce'}"><strong>{l s='Total of customers:' mod='btmailchimpecommerce'}</strong></span>
				</label>
				<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4">
					<input type="text" id="bt_nb-customers" name="bt_nb-customers" size="25" value="{$iTotal|intval}" class="disabled" />
				</div>
				...
				<div class="form-group">
					<label class="control-label col-xs-12 col-sm-12 col-md-4 col-lg-4">
						<span class="label-tooltip" data-toggle="tooltip" data-placement="bottom" title data-original-title="{l s='To avoid server resources limitations issues, the customers are sent in batches. Each batch contains the number of customers indicated here.' mod='btmailchimpecommerce'}"><strong>{l s='Number of customers per batch:' mod='btmailchimpecommerce'}</strong></span>
					</label>
				...
				<div id="bt_content_sync" style="display: none;" class="alert alert-info col-xs-12 col-sm-12 col-md-12 col-lg-12">
					...
					<h4>{l s='Customers synchronization' mod='btmailchimpecommerce'}</h4>
					<table>
						<tr>
							<td><b>{l s='Number of synchronized customers:' mod='btmailchimpecommerce'}</b>&nbsp;</td>
							<td>&nbsp;</td>
							<td><input type="text" size="5" name="bt_regen_counter" id="bt_regen_counter" value="0" />&nbsp;</td>
							<td>&nbsp;</td>
							<td>{l s='out of' mod='btmailchimpecommerce'}&nbsp;{$iTotal|intval} ({l s='customers in total' mod='btmailchimpecommerce'})</td>
						</tr>
					</table>
					<div class="clr_5"></div>
					<div class="center">
				...
				<div class="center">
					<a class="btn btn-success btn-lg" id="bt_sync_button" href="javascript:void(0);" onclick="if($('#bt_item_per_cycle').val() == 0 || $('#bt_item_per_cycle').val() == ''){literal}{{/literal}alert('{l s='You must fill in a value for the number of customers per batch!' mod='btmailchimpecommerce'}'); return false;{literal}}{/literal};aCustomerSync.iStep=$('#bt_item_per_cycle').val();$('#bt_content_sync').show();oMailchimp.synchronizeData(aCustomerSync);"><span class="icon-refresh"></span>&nbsp;{l s='Synchronize' mod='btmailchimpecommerce'}</a>
				</div>
			{else}
				<div class="form-group">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="alert alert-warning">
							{l s='There are no customers to synchronize!'  mod='btmailchimpecommerce'}
						</div>
					</div>
				</div>
			{/if}
		...
		<div id="bt_loading_div_sync" style="display: none;">
			...
			<div class="alert alert-info">
				...
				<p style="text-align: center !important;">{l s='Your customers synchronization is in progress...' mod='btmailchimpecommerce'}</p>
			</div>
		</div>
		{literal}
		<script type="text/javascript">
...
{else}
<div id="bt_div_customer_list_batch">
	<div id='{$sModuleName|escape:'htmlall':'UTF-8'}' class="bootstrap autoscroll">
		...
		<form class="form-horizontal col-xs-12  col-sm-12 col-md-12 col-lg-12 fancyform" action="{$sURI|escape:'htmlall':'UTF-8'}" method="post" id="bt_batch_form" name="bt_batch_form">
			...
			<input type="hidden" name="bt_sync_type" value="customer" />
			...
			{if empty($bCanClose)}
				...
				<div class="form-group">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="alert alert-warning">
							<b>{l s='The customers synchronization has begun, please just wait for the end of batches sending: the window will refresh itself when it\'s finished.' mod='btmailchimpecommerce'}</b>
							<div class="clr_10"></div>
							<p style="text-align: center !important;"><img src="{$sLoader|escape:'htmlall':'UTF-8'}" alt="Loading" /></p><div class="clr_20"></div>
							<div class="center"><strong><span id="bt_countdown_customer">{$iRefreshWaitingTime|intval}</span></strong> {l s='seconds' mod='btmailchimpecommerce'}</div>
						</div>
					</div>
				</div>
			{/if}
				...
				<div class="form-group">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="alert alert-success">
							<b>{$iTotal|intval}&nbsp;{l s='customers sent to MailChimp for processing' mod='btmailchimpecommerce'}</b>
						</div>
					</div>
				</div>
			{/if}
			...
			<h2>{l s='Customers processing' mod='btmailchimpecommerce'}</h2>
			<div class="clr_hr"></div>
			<div class="clr_20"></div>
					...
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<table class="table table-striped table-responsive">
							...
							<tr>
								...
								<th><span class="title_box center">{l s='Range of customers processed' mod='btmailchimpecommerce'}&nbsp;<span class="label-tooltip" data-toggle="tooltip" title data-original-title="{l s='This allows you to know which range of customers you have to synchronize again if errors are returned.' mod='btmailchimpecommerce'}">&nbsp;<span class="icon-question-sign"></span></span></span></th>
							</tr>
							</thead>
							<tbody>
			...
			{else}
				{l s='The execution of all the batches is finished. HOWEVER, to fully finish the process, please click on the "Finalize customers sync" green button below.' mod='btmailchimpecommerce'}
			{/if}
			...
			<div id="bt_loading_div_batch-resync" style="display: none;">
				...
				<div class="alert alert-info">
					<p style="text-align: center !important;"><img src="{$sLoader|escape:'htmlall':'UTF-8'}" alt="Loading" /></p><div class="clr_20"></div>
					<p style="text-align: center !important;">{l s='The form to resynchronize customers in error will soon be displayed...' mod='btmailchimpecommerce'}</p>
				</div>
			</div>
			...
			<div class="center">
				{if !empty($bCanClose)}
					<button id="bt_btn_finalize_customer" class="btn btn-success btn-lg" onclick="oMailchimp.form('bt_batch_form', '{$sURI|escape:'htmlall':'UTF-8'}', null, 'bt_batch_form', 'bt_batch_form', false, false, oBatchCallBack, 'batch', 'batch_finalize');return false;">{l s='Finalize customers sync' mod='btmailchimpecommerce'}</button>
				{else}
					<button id="bt_btn_refresh_customer" class="btn btn-success btn-lg" onclick="oMailchimp.clearTimeOut();$('#bt_loading_div_batch_reload').show();oMailchimp.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sReloadUrlParams|escape:'htmlall':'UTF-8'}', 'bt_div_customer_list_batch', 'bt_div_customer_list_batch', false, false, 'batch_reload', null, null);return false;">{l s='Refresh details' mod='btmailchimpecommerce'}</button>
				{/if}
			</div>
		</form>
		...
		<div id="bt_loading_div_batch_finalize" style="display: none;">
			...
			<div class="alert alert-info">
				<p style="text-align: center !important;"><img src="{$sLoader|escape:'htmlall':'UTF-8'}" alt="Loading" /></p><div class="clr_20"></div>
				<p style="text-align: center !important;">{l s='The finalization of customers synchronization is in progress...' mod='btmailchimpecommerce'}</p>
			</div>
		</div>
		...
		<script type="text/javascript">
			...
			{if empty($bCanClose)}
			{literal}
			oMailchimp.chrono('oMailchimp',{/literal}{$iRefreshWaitingTime|intval}{literal}, '#bt_countdown_customer', '#bt_btn_refresh_customer', 'click');
			{/literal}
			{else}
			$('#bt_btn_finalize_customer').trigger('click');
			{/if}
			{literal}
		</script>
		{/literal}
