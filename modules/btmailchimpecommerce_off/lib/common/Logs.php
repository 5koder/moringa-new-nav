...
class Logs
{
    ...
    /**
     * magic method initialize variables
     *
     * @param array $config
     */
    ...
    public function write($content)
    {
        ...
        foreach ($this->_aConfig['Pattern'] as $key) {
            $line .= (empty($line) ? '' : $this->_aConfig['Separator']) . $this->getValueOf($key);
        }
        // add content to log
        if (is_array($content)) {
        ...
        if (isset($this->_aConfig['MaxFileSize']) && file_exists($this->_aConfig['FilePath'])) {
            if (filesize($this->_aConfig['FilePath']) >= $this->_aConfig['MaxFileSize']) {
                $archiveName = substr($this->_aConfig['FilePath'], 0,
                        strrpos($this->_aConfig['FilePath'], '.')) . '-' . time() . substr($this->_aConfig['FilePath'],
