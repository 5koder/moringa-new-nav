...
class IqitAdditionalTabs extends Module implements WidgetInterface
{
    ...
    public function hookDisplayAdminProductsExtra($params)
    {
        ...
        if ($this->shouldRedirectToV2()) {
            return $this->display(__FILE__, 'views/templates/admin/bo_productab_v2.tpl');
        } else{
                ...
                foreach ($tabs as $key => $tab) {
                    if ($tab['title']) {
                        $array[] = (new ProductExtraContent())
                            ->setTitle($tab['title'])
                            ->addAttr(['class' => 'iqitadditionaltab iqitadditionaltab-'.$tab['id_iqitadditionaltab']])
    ...
    public function hookActionProductAdd($params)
    {
        if(isset($params['id_product_old'])){
            ...
            $tabsIds = IqitAdditionalTab::getIdTabs('product', $idProductOld, false);

            foreach ($tabsIds as $key => $tabId) {

                $tab = new IqitAdditionalTab((int)$tabId['id_iqitadditionaltab']);
                $tab->id_product =  $idProduct;
                $tab->add();
